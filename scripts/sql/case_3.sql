...
================================================================================
  Case 3 수익 분석 쿼리
================================================================================
  목적: 배송 대행 서비스의 CONSOLE/REPACK 패키지(상위 패키지)에 대한 수익 계산
  
  주요 계산:
  - 각종 수수료(storage, photo, repack 등)를 USD → KRW 환율로 변환
  - 배송 비용의 마크업을 포함한 총 수익 계산
  - 하위 패키지(INNER_PACKAGE)는 별도로 계산되지 않음 (상위 패키지만)
  
  필터 조건:
  - order_type: WE_SHIP (배송 대행)
  - package_type: CONSOLE 또는 REPACK (묶음 패키지)
  - 하위 패키지(INNER_PACKAGE)의 reference_type: 전부 ASN
...
SELECT 
    ...
    -- 거래 날짜 (UTC 기준, 패키지 단위)
    DATE(spn_u.trans_at_utc) AS trans_at_utc_package, 
    
    -- 패키지 고유 ID (상위 패키지 ID)
    -- CONSOLE, REPACK 패키지의 경우 여러 하위 패키지를 포함
    spn_u.package_id, 
    
    -- 하위 패키지 ID
    -- CONSOLE/REPACK의 경우 하위 패키지 ID 표시
    spn_u.inner_package_id,
    
    -- 하위 패키지 ID 목록 (콤마 구분)
    -- 해당 상위 패키지에 포함된 모든 하위 패키지 ID를 콤마로 연결
    -- 예: "123456,123457,123458"
    (
        SELECT STRING_AGG(DISTINCT CAST(spn_inner.package_id AS STRING), ',' ORDER BY CAST(spn_inner.package_id AS STRING))
        FROM `da-project-472406.data_warehouse.shipment_package_new` spn_inner
        WHERE spn_inner.package_id = spn_u.package_id
          AND spn_inner.package_type = 'INNER'
    ) AS inner_package_ids,
    ...
    -- 주문 유형
    -- 상위 패키지의 order_type이 NULL인 경우 하위 패키지의 order_type을 참조
    -- 하위 패키지의 고유값들을 콤마로 구분하여 표시
    COALESCE(
        spn_u.order_type,
        (
            SELECT STRING_AGG(DISTINCT spn_inner.order_type, ',' ORDER BY spn_inner.order_type)
            FROM `da-project-472406.data_warehouse.shipment_package_new` spn_inner
            WHERE spn_inner.package_id = spn_u.package_id
              AND spn_inner.package_type = 'INNER'
              AND spn_inner.order_type IS NOT NULL
        )
    ) AS order_type, 
    
    -- 패키지 타입
    -- CONSOLE: 여러 패키지를 하나로 묶음
    -- REPACK: 재포장된 패키지
    spn_u.package_type, 
    ...
    ROUND(
      ...
      ) * COALESCE(c_u.usd_krw, 1450)  -- 시스템 환율 적용 (실질 수익 기준)
...
WHERE 
    -- 패키지 타입: CONSOLE 또는 REPACK (묶음 패키지만)
    -- CONSOLE: 여러 패키지를 하나로 묶음
    -- REPACK: 재포장된 패키지
    spn_u.package_type IN ('CONSOLE', 'REPACK')
    
    -- 하위 패키지의 reference_type이 전부 ASN인 경우만 포함
    -- 모든 하위 패키지가 ASN(Advanced Shipping Notice)으로 등록된 경우
    AND NOT EXISTS (
        SELECT 1
        FROM `da-project-472406.data_warehouse.shipment_package_new` spn_inner
        WHERE spn_inner.package_id = spn_u.package_id
